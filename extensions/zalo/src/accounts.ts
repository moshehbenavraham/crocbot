import type { crocbotConfig } from "clawdbot/plugin-sdk";
...
export function listZaloAccountIds(cfg: crocbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: crocbotConfig): string {
...
function mergeZaloAccountConfig(cfg: crocbotConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: crocbotConfig): ResolvedZaloAccount[] {
