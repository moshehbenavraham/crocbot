...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as crocbotConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as crocbotConfig),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as crocbotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as crocbotConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as crocbotConfig).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as crocbotConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as crocbotConfig });
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZaloProvider({
        token,
        account,
        config: ctx.cfg as crocbotConfig,
        runtime: ctx.runtime,
