...
function resolvecrocbotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.crocbot_ROOT?.trim() || process.env.CLAWDBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["crocbot", "crocbot"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set crocbot_ROOT (or legacy CLAWDBOT_ROOT) to the package root.",
  );
}

async function importCoreExtensionAPI(): Promise<CoreAgentDeps> {
  const distPath = path.join(resolvecrocbotRoot(), "dist", "extensionAPI.js");
