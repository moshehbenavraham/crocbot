...
metadata: {"crocbot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == crocbot `sessions_spawn`
- **File I/O** == crocbot `read`/`write`
- **Remote fetch** == crocbot `web_fetch` (or `exec` with curl when POST is required)
