...
// NOTE: This extension is intended to be bundled with crocbot.
// When running from source (tests/dev), crocbot internals live under src/.
...
export function createLlmTaskTool(api: crocbotPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-llm-task-"));
