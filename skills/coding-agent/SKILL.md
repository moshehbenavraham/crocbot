...
**⚠️ CRITICAL: Never review PRs in crocbot's own project folder!**
...
For long-running background tasks, append a wake trigger to your prompt so crocbot gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
crocbot gateway wake --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: crocbot gateway wake --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
