...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/crocbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "crocbot Re-Auth" -c "After getting key, run: ssh $SERVER '~/crocbot/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
