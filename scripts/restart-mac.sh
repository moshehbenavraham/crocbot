#!/usr/bin/env bash
# Reset crocbot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWDBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="crocbot.app/Contents/MacOS/crocbot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/crocbot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/crocbot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/crocbot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.clawdbot.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/crocbot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWDBOT_RESTART_LOG:-/tmp/crocbot-restart.log}"
NO_SIGN=0
SIGN=0
...
log "==> Killing existing crocbot instances"
kill_all_crocbot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product crocbot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/crocbot.app" ]]; then
    APP_BUNDLE="/Applications/crocbot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/crocbot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/crocbot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/crocbot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWDBOT_APP_BUNDLE to your installed crocbot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".clawdbot", "crocbot.json"), "utf8");
