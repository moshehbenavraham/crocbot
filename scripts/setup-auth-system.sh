#!/bin/bash
# Setup crocbot Auth Management System
...
echo "=== crocbot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'crocbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  crocbot message: Send warning via crocbot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/crocbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/crocbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/crocbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now crocbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status crocbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
