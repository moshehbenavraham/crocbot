...
IMAGE_NAME="crocbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/crocbot-bin:$PATH"
  mkdir -p /tmp/crocbot-bin

  cat > /tmp/crocbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/crocbot-bin/systemctl

  cat > /tmp/crocbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/crocbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/crocbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/crocbot/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/crocbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/crocbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
