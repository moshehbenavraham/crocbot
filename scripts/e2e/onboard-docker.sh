...
IMAGE_NAME="crocbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/crocbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/crocbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/crocbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/crocbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/croc"
    config_path="$HOME/.crocbot/crocbot.json"
    sessions_dir="$HOME/.crocbot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    export HOME="$home_dir"
    mkdir -p "$HOME/.crocbot"
    # Seed a remote config to exercise reset path.
    cat > "$HOME/.crocbot/crocbot.json" <<'"'"'JSON'"'"'
{
