...
#
# Suggested cron: */30 * * * * /home/admin/crocbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send crocbot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., crocbot-alerts)
...
send_notification() {
    ...
    # Send via crocbot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use crocbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via crocbot to $NOTIFY_PHONE..."
            crocbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! crocbot is down. Run: ssh l36 '~/crocbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
