...
ZIP=${1:?"Usage: $0 crocbot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/crocbot/crocbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^crocbot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/crocbot/crocbot/releases/download/v${VERSION}/"}
