...
# Build and bundle crocbot into a minimal .app we can open.
# Outputs to dist/crocbot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/crocbot.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/crocbot/crocbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/crocbot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :crocbotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :crocbotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/crocbot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/crocbot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/crocbot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/crocbot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/crocbot/Resources/crocbot.icns" "$APP_ROOT/Contents/Resources/crocbot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/crocbot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying crocbotKit resources"
crocbotKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/crocbotKit_crocbotKit.bundle"
if [ -d "$crocbotKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/crocbotKit_crocbotKit.bundle"
  cp -R "$crocbotKIT_BUNDLE" "$APP_ROOT/Contents/Resources/crocbotKit_crocbotKit.bundle"
else
  echo "WARN: crocbotKit resource bundle not found at $crocbotKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running crocbot"
killall -q crocbot 2>/dev/null || true
