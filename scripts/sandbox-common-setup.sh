...
BASE_IMAGE="${BASE_IMAGE:-crocbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-crocbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=crocbot.sandbox=1)
NOTE
