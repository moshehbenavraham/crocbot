...
Matrix is an open, decentralized messaging protocol. crocbot connects as a Matrix **user**
...
```bash
crocbot plugins install @crocbot/matrix
```

Local checkout (when running from a git repo):

```bash
crocbot plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `crocbot plugins install @crocbot/matrix`
   - From a local checkout: `crocbot plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: crocbot calls the same
...
- On first connection, crocbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  crocbot logs a warning.
...
- Approve via:
  - `crocbot pairing list matrix`
  - `crocbot pairing approve matrix <CODE>`
...
- On startup, crocbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
