...
read_when:
  - You want crocbot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables crocbot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`crocbot onboard`) and `crocbot channels add` list optional channel plugins.
...
```bash
crocbot plugins install @crocbot/nostr
```

Use a local checkout (dev workflows):

```bash
crocbot plugins install --link <path-to-crocbot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "crocbot@example.com",
        "lud16": "crocbot@example.com"
      }
    }
  }
}
