...
```bash
crocbot plugins install @crocbot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
crocbot plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "crocbot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `crocbot pairing list nextcloud-talk`
  - `crocbot pairing approve nextcloud-talk <CODE>`
