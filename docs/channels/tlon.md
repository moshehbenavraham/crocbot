...
Tlon is a decentralized messenger built on Urbit. crocbot connects to your Urbit ship and can
...
```bash
crocbot plugins install @crocbot/tlon
```

Local checkout (when running from a git repo):

```bash
crocbot plugins install ./extensions/tlon
```
...
Use these with `crocbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, crocbot replies in-thread.
