...
```bash
crocbot plugins install @crocbot/msteams
```

Local checkout (when running from a git repo):
```bash
crocbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
crocbot will offer the local install path automatically.
...
3) Configure crocbot with those credentials.
...
## Goals
- Talk to crocbot via Teams DMs, group chats, or channels.
...
- On startup, crocbot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.clawdbot/crocbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring crocbot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `crocbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `crocbot plugins install @crocbot/msteams`
   - From a local checkout: `crocbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "crocbot in Teams", "full": "crocbot in Teams" },
...
crocbot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, crocbot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/crocbotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
crocbot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `crocbot message poll --channel msteams --target conversation:<id> ...`
...
**CLI:**
```bash
crocbot message send --channel msteams \
...
crocbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
crocbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
crocbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
crocbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
