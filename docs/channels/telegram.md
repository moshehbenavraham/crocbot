...
- If Telegram rejects the HTML payload, crocbot retries the same message as plain text.

## Commands (native + custom)
crocbot registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; crocbot does not implement them unless you handle them elsewhere.
...
This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). crocbot can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. crocbot:
...
Private chats can include `message_thread_id` in some edge cases. crocbot keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2) Run `crocbot logs --follow` and look for `from.id`.
...
crocbot defaults to audio files for backward compatibility.
...
crocbot supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, crocbot handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, crocbot caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.crocbot/telegram/sticker-cache.json`
...
crocbot uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, crocbot:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by crocbot)
...
- Example: `crocbot message send --channel telegram --target 123456789 --message "hi"`.
...
- `crocbot channels status` shows a warning when config expects unmentioned group messages.
- `crocbot channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `crocbot logs --follow` (look for "skipping group message")
...
- Upgrade to a crocbot build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
...
- `commands.native` (defaults to `"auto"` → on for Telegram), `commands.text`, `commands.useAccessGroups` (command behavior). Override with `channels.telegram.commands.native`.
