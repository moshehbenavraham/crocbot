...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `crocbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `crocbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=crocbot.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${CROCBOT_STATE_DIR:-$HOME/.crocbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
