...
Accessed via `crocbotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: crocbotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: crocbotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: crocbotConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: crocbotConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: crocbotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: crocbotConfig): string;
  };
};
```
...
- Plugins declare a required runtime range (e.g., `crocbotRuntime: ">=2026.2.0"`).
