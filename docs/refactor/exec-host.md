...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.crocbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.crocbot/node.json` (node id + pairing token).
...
Path: `~/.crocbot/exec-approvals.json`
...
### IPC
- Unix socket at `~/.crocbot/exec-approvals.sock` (0600).
