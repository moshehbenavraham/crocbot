...
Deploy crocbot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/crocbot/crocbot" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run crocbot:
...
4. Optionally configure Telegram (add your bot token)
5. Click **Run setup**

### Access the Control UI

The web dashboard is available at `https://<your-service>.onrender.com/crocbot`.
...
If you use the original crocbot repository, Render will not auto-deploy your crocbot. To update it, run a manual Blueprint sync from the dashboard.
...
For crocbot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
