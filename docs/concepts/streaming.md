...
crocbot has two separate “streaming” layers:
...
When block streaming is enabled, crocbot can **merge consecutive block chunks**
...
- Default coalesce `minChars` is bumped to 1500 for some channels unless overridden.
...
When draft streaming is active, crocbot disables block streaming for that reply to avoid double-streaming.
