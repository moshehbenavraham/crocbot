---
summary: "Gmail Pub/Sub push wired into crocbot webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to crocbot
  - Setting up Pub/Sub push for agent wake
---

# Gmail Pub/Sub -> crocbot

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> crocbot webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- crocbot hooks enabled (see [Webhooks](/automation/webhook)).
...
- Writes `hooks.gmail` config for `crocbot webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, crocbot automatically sets
...
- Set `CROCBOT_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
- `--hook-url` points to crocbot `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to crocbot.

Recommended: `crocbot webhooks gmail run` wraps the same flow and auto-renews the watch.
