...
crocbot features that can generate provider usage or paid API calls.
...
crocbot can pick up credentials from:
...
### 8) Model scan / probe
`crocbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
