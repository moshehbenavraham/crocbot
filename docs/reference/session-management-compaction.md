...
This document explains how crocbot manages sessions end-to-end:
...
crocbot persists sessions in two layers:
...
crocbot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (crocbot consumes the events, but Pi decides when to compact).
...
crocbot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, crocbot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, crocbot leaves it alone.
...
- `crocbot status` (CLI)
- `crocbot sessions` / `sessions --json`
...
crocbot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- crocbot strips/suppresses this in the delivery layer.

As of `2026.1.10`, crocbot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
crocbot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but crocbot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `crocbot status`.
