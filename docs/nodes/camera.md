...
crocbot supports **camera capture** for agent workflows:
...
- **Settings → General → Allow Camera** (`crocbot.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `crocbot` CLI to invoke camera commands on the macOS node.

Examples:

```bash
crocbot nodes camera list --node <id>            # list camera ids
crocbot nodes camera snap --node <id>            # prints MEDIA:<path>
crocbot nodes camera snap --node <id> --max-width 1280
crocbot nodes camera snap --node <id> --delay-ms 2000
crocbot nodes camera snap --node <id> --device-id <id>
crocbot nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
crocbot nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
crocbot nodes camera clip --node <id> --device-id <id>
crocbot nodes camera clip --node <id> --no-audio
```

Notes:
- `crocbot nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
crocbot nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
