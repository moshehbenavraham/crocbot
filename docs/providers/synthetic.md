---
summary: "Use Synthetic's Anthropic-compatible API in crocbot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. crocbot registers it as the
...
```bash
crocbot onboard --auth-choice synthetic-api-key
```
...
Note: crocbot's Anthropic client appends `/v1` to the base URL, so use
