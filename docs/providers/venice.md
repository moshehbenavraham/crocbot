---
summary: "Use Venice AI privacy-focused models in crocbot"
read_when:
  - You want privacy-focused inference in crocbot
...
```bash
crocbot onboard --auth-choice venice-api-key
```
...
```bash
crocbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, crocbot shows all available Venice models. Pick based on your needs:
...
```bash
crocbot models set venice/claude-opus-45
crocbot models set venice/llama-3.3-70b
```

List all available models:

```bash
crocbot models list | grep venice
```

## Configure via `crocbot configure`

1. Run `crocbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
crocbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
crocbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
crocbot chat --model venice/claude-opus-45

# Use uncensored model
crocbot chat --model venice/venice-uncensored

# Use vision model with image
crocbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
crocbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `crocbot models list` to see currently available models. Some models may be temporarily offline.
