---
summary: "Model providers (LLMs) supported by crocbot"
read_when:
...
crocbot can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `crocbot onboard`).
