...
- During heartbeat runs, crocbot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `crocbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, crocbot skips the heartbeat run entirely (no model call).
...
```bash
crocbot system event --text "Check for urgent follow-ups" --mode now
```
