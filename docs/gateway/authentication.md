...
crocbot supports OAuth and API keys for model providers. For Anthropic
...
2) Put it on the **gateway host** (the machine running `crocbot gateway`).
...
`~/.crocbot/.env` so the daemon can read it:
...
API keys for daemon use: `crocbot onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.crocbot/.env`, systemd/launchd).
...
```bash
crocbot models auth paste-token --provider anthropic
crocbot models auth paste-token --provider openrouter
```

Automation-friendly check (exit `1` when expired/missing, `2` when expiring):

```bash
crocbot models status --check
```
...
Run `crocbot models status` to confirm which profile is expiring. If the profile
