...
`crocbot gateway` supports a per-gateway style switch:
...
crocbot gateway --verbose --ws-log compact

# show all WS traffic (full meta)
crocbot gateway --verbose --ws-log full
```
...
- **Message bodies** are logged at `debug` (use `--verbose` to see them)
