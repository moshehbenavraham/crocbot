...
crocbot can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. crocbot requires a shared password.
...
- `token` (default when `CROCBOT_GATEWAY_TOKEN` is set)
- `password` (shared secret via `CROCBOT_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. crocbot verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
crocbot only treats a request as Serve when it arrives from loopback with
...
Prefer `CROCBOT_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
crocbot gateway --tailscale serve
crocbot gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want crocbot to undo `tailscale serve`
