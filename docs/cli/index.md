---
summary: "crocbot CLI reference for `crocbot` commands, subcommands, and options"
read_when:
...
```
crocbot [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
- `crocbot security audit` — audit config + local state for common security foot-guns.
- `crocbot security audit --deep` — best-effort live Gateway probe.
- `crocbot security audit --fix` — tighten safe defaults and chmod state/config.

## Plugins

Manage extensions and their config:

- `crocbot plugins list` — discover plugins (use `--json` for machine output).
- `crocbot plugins info <id>` — show details for a plugin.
- `crocbot plugins install <path|.tgz|npm-spec>` — install a plugin (or add a plugin path to `plugins.load.paths`).
...
- `crocbot memory status` — show index stats.
- `crocbot memory index` — reindex memory files.
- `crocbot memory search "<query>"` — semantic search over memory.
...
### `config`
Non-interactive config helpers (get/set/unset). Running `crocbot config` with no
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `crocbot health` or `crocbot status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `crocbot doctor`).
...
### Usage tracking
crocbot can surface provider usage/quota when OAuth/API creds are available.
...
```bash
crocbot logs --follow
crocbot logs --limit 200
crocbot logs --plain
crocbot logs --json
crocbot logs --no-color
```
...
### `models` (root)
`crocbot models` is an alias for `models status`.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`crocbot browser`](/cli/browser) and the [Browser tool](/tools/browser).
