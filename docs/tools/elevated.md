...
- Sender allowlist: `tools.elevated.allowFrom` with per-provider allowlists (e.g. `telegram`).
...
- Telegram fallback: if `tools.elevated.allowFrom.telegram` is omitted, the `channels.telegram.allowFrom` list is used as a fallback. Set `tools.elevated.allowFrom.telegram` (even `[]`) to override. Per-agent allowlists do **not** use the fallback.
