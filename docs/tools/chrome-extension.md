---
summary: "Chrome extension: let crocbot drive your existing Chrome tab"
read_when:
...
The crocbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate croc-managed Chrome profile.
...
crocbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the crocbot release (npm package) as static files. There is no separate “build” step.

After upgrading crocbot:
- Re-run `crocbot browser extension install` to refresh the installed files under your crocbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

crocbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want crocbot to control.
...
- `ON`: attached; crocbot can drive that tab.
...
Debugging: `crocbot sandbox explain`
...
`crocbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `crocbot browser extension install` first to copy the extension to a stable location under your crocbot state directory.
...
- **This is not isolated** like the dedicated croc-managed profile.
