...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `crocbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, crocbot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`crocbot node run`).
...
Notes:
- Requires a Brave API key (recommended: `crocbot configure --section web`, or set `BRAVE_API_KEY`).
...
### `message`
Send messages and channel actions via Telegram.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `crocbot gateway` restart in-place)
