---
title: Lobster
summary: "Typed workflow runtime for crocbot with resumable approval gates."
description: Typed workflow runtime for crocbot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets crocbot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: crocbot runs one Lobster tool call and gets a structured result.
...
crocbot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In crocbot tool calls, set `pipeline` to the file path.
...
plugin tools (like `lobster`), crocbot keeps core tools enabled. To restrict core
