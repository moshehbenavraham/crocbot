---
summary: "crocbot logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
crocbot routes macOS app logs through swift-log (unified logging by default) and can write a local, rotating file log to disk when you need a durable capture.

- Verbosity: **Debug pane → Logs → App logging → Verbosity**
- Enable: **Debug pane → Logs → App logging → “Write rolling diagnostics log (JSONL)”**
- Location: `~/Library/Logs/crocbot/diagnostics.jsonl` (rotates automatically; old files are suffixed with `.1`, `.2`, …)
...
## Enable for crocbot (`com.clawdbot`)
