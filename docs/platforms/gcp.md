---
summary: "Run crocbot Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want crocbot running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
---

# crocbot on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent crocbot Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "crocbot 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
```bash
gcloud projects create my-crocbot-project --name="crocbot Gateway"
gcloud config set project my-crocbot-project
```
...
```bash
gcloud compute instances create crocbot-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh crocbot-gateway --zone=us-central1-a
```
...
## 6) Clone the crocbot repository
...
```bash
gcloud compute ssh crocbot-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop crocbot-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type crocbot-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start crocbot-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ```bash
   gcloud iam service-accounts create crocbot-deploy \
     --display-name="crocbot Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):
   ```bash
   gcloud projects add-iam-policy-binding my-crocbot-project \
     --member="serviceAccount:crocbot-deploy@my-crocbot-project.iam.gserviceaccount.com" \
