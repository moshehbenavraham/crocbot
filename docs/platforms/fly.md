---
title: Fly.io
description: Deploy crocbot on Fly.io
---
...
fly apps create my-crocbot

# Create a persistent volume (1GB is usually enough)
fly volumes create crocbot_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `crocbot.json` where they could be accidentally exposed or logged.
...
Or visit `https://my-crocbot.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/crocbot.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/crocbot.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/crocbot.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/crocbot.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/crocbot.json"
```
...
```bash
# List current IPs
fly ips list -a my-crocbot

# Release public IPs
fly ips release <public-ipv4> -a my-crocbot
fly ips release <public-ipv6> -a my-crocbot
...
fly ips allocate-v6 --private -a my-crocbot
```
...
fly proxy 3000:3000 -a my-crocbot
