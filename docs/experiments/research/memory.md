---
summary: "Research notes: offline memory system for Croc workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/croc) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep crocbot integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Croc-style workspace (`agents.defaults.workspace`, default `~/croc`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/croc` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Croc to see every session.
...
- O(c=0.95) @Peter: Prefers concise replies (&lt;1500 chars) on Telegram; long content goes into files.
```
...
Recommendation: **deep integration in crocbot**, but keep a separable core library.

### Why integrate into crocbot?
- crocbot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `crocbot memory recall "…" --k 25 --since 30d`
  - `crocbot memory reflect --since 7d`
