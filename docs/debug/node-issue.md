...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in crocbot
---

# Node + tsx "__name is not a function" crash

## Summary
Running crocbot via Node with `tsx` fails at startup with:

```
[crocbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `crocbot status` and `gateway:watch` worked.
