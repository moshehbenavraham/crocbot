...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = crocbotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = crocbotCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = crocbotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = crocbotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__crocbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: crocbotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for crocbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case crocbotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case crocbotCanvasCommand.present.rawValue,
            ...
            case crocbotCanvasA2UICommand.reset.rawValue,
            ...
            case crocbotCameraCommand.list.rawValue,
                 crocbotCameraCommand.snap.rawValue,
                 crocbotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case crocbotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: crocbotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: crocbotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(crocbotLocationGetParams.self, from: req.paramsJSON)) ??
            crocbotLocationGetParams()
        ...
        let payload = crocbotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case crocbotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(crocbotCanvasPresentParams.self, from: req.paramsJSON)) ??
                crocbotCanvasPresentParams()
        ...
        case crocbotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case crocbotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(crocbotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case crocbotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(crocbotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case crocbotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(crocbotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case crocbotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.clawdbotA2UI) return JSON.stringify({ ok: false, error: "missing crocbotA2UI" });
              return JSON.stringify(globalThis.clawdbotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case crocbotCanvasA2UICommand.push.rawValue, crocbotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == crocbotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(crocbotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try crocbotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(crocbotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try crocbotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.clawdbotA2UI) return JSON.stringify({ ok: false, error: "missing crocbotA2UI" });
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case crocbotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case crocbotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(crocbotCameraSnapParams.self, from: req.paramsJSON)) ??
                crocbotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case crocbotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(crocbotCameraClipParams.self, from: req.paramsJSON)) ??
                crocbotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(crocbotScreenRecordParams.self, from: req.paramsJSON)) ??
            crocbotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> crocbotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return crocbotLocationMode(rawValue: raw) ?? .off
    }
