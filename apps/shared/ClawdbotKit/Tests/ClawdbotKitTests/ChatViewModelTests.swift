...
private final class TestChatTransport: @unchecked Sendable, crocbotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [crocbotChatHistoryPayload]
    private let sessionsResponses: [crocbotChatSessionsListResponse]

    private let stream: AsyncStream<crocbotChatTransportEvent>
    private let continuation: AsyncStream<crocbotChatTransportEvent>.Continuation

    init(
        historyResponses: [crocbotChatHistoryPayload],
        sessionsResponses: [crocbotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<crocbotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> crocbotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? crocbotChatHistoryPayload(
        ...
        attachments _: [crocbotChatAttachmentPayload]) async throws -> crocbotChatSendResponse
    {
        ...
        return crocbotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> crocbotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? crocbotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: crocbotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = crocbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = crocbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { crocbotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = crocbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = crocbotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = crocbotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { crocbotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
