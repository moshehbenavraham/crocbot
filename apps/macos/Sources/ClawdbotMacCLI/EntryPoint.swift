...
struct crocbotMacCLI {
    static func main() async {
        ...
        default:
            fputs("crocbot-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      crocbot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      crocbot-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      crocbot-mac connect
      crocbot-mac connect --url ws://127.0.0.1:18789 --json
      crocbot-mac discover --timeout 3000 --json
      crocbot-mac wizard --mode local
    """)
}
