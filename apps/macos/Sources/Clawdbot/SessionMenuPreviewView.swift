...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> crocbotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: crocbotSessionPreviewEntry,
    ...
    private static func cache(payload: crocbotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: crocbotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [crocbotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [crocbotKit.AnyCodable]) -> [crocbotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(crocbotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: crocbotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: crocbotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: crocbotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: crocbotChatMessage) -> String? {
