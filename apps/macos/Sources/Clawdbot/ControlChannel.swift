...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: crocbotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: crocbotKit.AnyCodable]()) {
                $0[$1.key] = crocbotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: crocbotProtocol.AnyCodable?) -> [String: crocbotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: crocbotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: crocbotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: crocbotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("crocbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("crocbot.control.agent")
}
