...
enum CommandResolver {
    private static let projectRootDefaultsKey = "crocbot.gatewayProjectRootPath"
    private static let helperName = "crocbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/crocbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/crocbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let crocbotPaths = self.clawdbotManagedPaths(home: home)
        if !crocbotPaths.isEmpty {
            extras.insert(contentsOf: crocbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + crocbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func crocbotManagedPaths(home: URL) -> [String] {
    ...
    static func crocbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectcrocbotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/crocbot.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnycrocbotInvoker(searchPaths: [String]? = nil) -> Bool {
    ...
    static func crocbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let crocbotPath = self.projectcrocbotExecutable(projectRoot: root) {
                return [crocbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "crocbot", subcommand] + extraArgs
            }
            if let crocbotPath = self.clawdbotExecutable(searchPaths: searchPaths) {
                return [crocbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            crocbot entrypoint missing (looked for dist/index.js or bin/crocbot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to crocbotCommand; keep it as node alias.
    static func crocbotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real crocbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/crocbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v crocbot >/dev/null 2>&1; then
          CLI="$(command -v crocbot)"
          crocbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/crocbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/crocbot.js"
            node "$PRJ/bin/crocbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent crocbot"
          pnpm --silent crocbot \(quotedArgs);
        else
          echo "crocbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? crocbotConfigFile.loadDict()
