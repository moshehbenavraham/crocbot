...
enum GatewayEnvironment {
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = crocbotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        ...
        case let .success(runtime):
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "crocbot CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "crocbot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "crocbot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "crocbot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "crocbot@\(target)"])
            }

        statusHandler("Installing crocbot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed crocbot@\(target)")
        } else {
