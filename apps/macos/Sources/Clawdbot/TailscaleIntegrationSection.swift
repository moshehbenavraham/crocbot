...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawdbot/crocbot.json. Prefer CLAWDBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawdbot/crocbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawdbot/crocbot.json. Restart the gateway to apply."
        }
