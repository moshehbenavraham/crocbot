...
function hasConfigGatewayCreds(cfg: crocbotConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const lines = [
    ...
    envToken ? "- `CROCBOT_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword ? "- `CROCBOT_GATEWAY_PASSWORD` is set; it overrides config passwords." : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv CROCBOT_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv CROCBOT_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
