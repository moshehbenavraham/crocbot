...
describe("onboard (non-interactive): Vercel AI Gateway", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-onboard-gateway-"));
    process.env.HOME = tempHome;
    process.env.CROCBOT_STATE_DIR = tempHome;
    process.env.CROCBOT_CONFIG_PATH = path.join(tempHome, "crocbot.json");
    vi.resetModules();
