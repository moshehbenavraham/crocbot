...
function resolveMode(cfg: crocbotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("crocbot doctor");

  const root = await resolvecrocbotPackageRoot({
  ...
  let cfg: crocbotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("crocbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("crocbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("crocbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("crocbot doctor --fix")}" to apply changes.`);
  }
