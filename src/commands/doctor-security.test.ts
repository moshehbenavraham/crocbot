...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    prevPassword = process.env.CROCBOT_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) delete process.env.CROCBOT_GATEWAY_PASSWORD;
    else process.env.CROCBOT_GATEWAY_PASSWORD = prevPassword;
  });
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CROCBOT_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as crocbotConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
