...
export function listAgentEntries(cfg: crocbotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: crocbotConfig, agentId: string) {
...
function resolveAgentModel(cfg: crocbotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: crocbotConfig): AgentSummary[] {
