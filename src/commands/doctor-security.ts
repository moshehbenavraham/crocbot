...
import type { crocbotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: crocbotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("crocbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("crocbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("crocbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("crocbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("crocbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
