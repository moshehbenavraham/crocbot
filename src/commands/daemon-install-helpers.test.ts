...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/crocbot/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\crocbot\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/crocbot/dist/cli/index.js"])).toBe(false);
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns reinstall hint", () => {
    expect(gatewayInstallErrorHint()).toMatch(
      /(?:crocbot|crocbot)( --profile isolated)? gateway install/,
    );
  });
});
