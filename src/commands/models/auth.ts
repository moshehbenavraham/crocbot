...
import { readConfigFileSnapshot, type crocbotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: crocbotConfig, model: string): crocbotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("crocbot plugins install")}\`.`,
    );
  }
