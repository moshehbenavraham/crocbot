...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvecrocbotAgentDir: vi.fn().mockReturnValue("/tmp/crocbot-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvecrocbotAgentDir: mocks.resolvecrocbotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/crocbot-agent/auth-profiles.json");
