...
beforeEach(() => {
  ...
  resolvecrocbotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CROCBOT_STATE_DIR;
  originalUpdateInProgress = process.env.CROCBOT_UPDATE_IN_PROGRESS;
  process.env.CROCBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "crocbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CROCBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CROCBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.CROCBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolvecrocbotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadcrocbotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/crocbot-root.js", () => ({
  resolvecrocbotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "crocbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CROCBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CROCBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/crocbot.json",
      exists: true,
      raw: "{}",
    ...
    const prevToken = process.env.CROCBOT_GATEWAY_TOKEN;
    process.env.CROCBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
