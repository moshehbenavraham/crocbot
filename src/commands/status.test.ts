...
beforeAll(() => {
  previousProfile = process.env.CROCBOT_PROFILE;
  process.env.CROCBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.CROCBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/crocbot-root.js", () => ({
  resolvecrocbotPackageRoot: vi.fn().mockResolvedValue("/tmp/crocbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/crocbot/pnpm-lock.yaml",
      markerPath: "/tmp/crocbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("crocbot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("crocbot status --all") ||
          l.includes("crocbot --profile isolated status --all") ||
          l.includes("crocbot status --all") ||
          l.includes("crocbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CROCBOT_GATEWAY_TOKEN = "abcd1234";
    try {
