...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      ...
      process.env.CROCBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.CROCBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under CROCBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-oauth-"));
    process.env.CROCBOT_STATE_DIR = tempStateDir;
    process.env.CROCBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CROCBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CROCBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-minimax-"));
    process.env.CROCBOT_STATE_DIR = tempStateDir;
    process.env.CROCBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CROCBOT_AGENT_DIR;
