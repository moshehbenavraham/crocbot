...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CROCBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CROCBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CROCBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("CROCBOT_GATEWAY_PASSWORD");
  });
