  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.crocbot/logs/crocbot.log" },
        hooks: {
          path: "~/.crocbot/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.crocbot/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".crocbot", "logs", "crocbot.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".crocbot", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".crocbot", "telegram.token"),
      );
