...
function isTelegramConfigured(cfg: crocbotConfig, env: NodeJS.ProcessEnv): boolean {
...
function collectModelRefs(cfg: crocbotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: crocbotConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(
  ...
  for (const channelId of channelIds) {
    ...
    if (coreChannelIds.has(channelId)) {
      continue;
    }
...
function isPluginExplicitlyDisabled(cfg: crocbotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: crocbotConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: crocbotConfig, pluginId: string): crocbotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
