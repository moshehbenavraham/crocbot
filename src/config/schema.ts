...
import { crocbotSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof crocbotSchema.toJSONSchema>;
...
const FIELD_LABELS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "Gateway HTTP Base Path",
  "gateway.controlUi.allowInsecureAuth": "Allow Insecure Gateway Auth",
  "gateway.controlUi.dangerouslyDisableDeviceAuth": "Dangerously Disable Gateway Device Auth",
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when crocbot writes the config.",
  ...
  "gateway.controlUi.basePath": "Optional URL prefix for the gateway HTTP server (e.g. /crocbot).",
  "gateway.controlUi.allowInsecureAuth":
    "Allow gateway auth over insecure HTTP (token-only; not recommended).",
  "gateway.controlUi.dangerouslyDisableDeviceAuth":
    "DANGEROUS. Disable gateway device identity checks (token/password only).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $CROCBOT_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.crocbot/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `crocbot plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.crocbot/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/crocbot",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/croc.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = crocbotSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "crocbotConfig";
