...
import type { crocbotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: crocbotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: crocbotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = crocbotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as crocbotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as crocbotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as crocbotConfig)),
    ),
  };
}
