...
export const handleTtsCommands: CommandHandler = async (params, allowTextCommands) => {
      ...
      return {
        ...
        reply: {
          text:
            ...
            `OpenAI key: ${hasOpenAI ? "configured" : "missing"}\n` +
            `ElevenLabs key: ${hasElevenLabs ? "configured" : "missing"}\n` +
            `Edge enabled: ${hasEdge ? "yes" : "no"}\n` +
  ...
  if (action === "status") {
    ...
    if (last) {
      ...
      lines.push("");
      lines.push(`Last attempt (${timeAgo}s ago): ${last.success ? "success" : "failed"}`);
