...
export function listIMessageAccountIds(cfg: crocbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: crocbotConfig): string {
...
function mergeIMessageAccountConfig(cfg: crocbotConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: crocbotConfig): ResolvedIMessageAccount[] {
