...
export function registerNodeCli(program: Command) {
  const node = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "aiwithapex.mintlify.app/cli/node")}\n`,
    );
  ...
  node
    .command("install")
    .description("Install the node host service (systemd)")
    .option("--host <host>", "Gateway host")
    .option("--port <port>", "Gateway port")
    .option("--tls", "Use TLS for the gateway connection", false)
    .option("--tls-fingerprint <sha256>", "Expected TLS certificate fingerprint (sha256)")
  ...
  node
    .command("uninstall")
    .description("Uninstall the node host service (systemd)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
  ...
  node
    .command("stop")
    .description("Stop the node host service (systemd)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
  ...
  node
    .command("restart")
    .description("Restart the node host service (systemd)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
