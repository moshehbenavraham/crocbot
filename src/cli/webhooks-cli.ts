...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.github.com/moshehbenavraham/crocbot/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + crocbot hooks")
    ...
    .option("--hook-url <url>", "crocbot hook URL")
    .option("--hook-token <token>", "crocbot hook token")
