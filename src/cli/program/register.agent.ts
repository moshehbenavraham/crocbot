...
${formatHelpExamples([
  ['crocbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['crocbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['crocbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "aiwithapex.mintlify.app/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "aiwithapex.mintlify.app/cli/agents")}\n`,
    );
