...
function appendCrocHubHint(output: string, json?: boolean): string {
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("crocbot skills list")}\` to see all skills.`
      : "No skills found.";
    return appendCrocHubHint(message, opts.json);
  }
  ...
  return appendCrocHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendCrocHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("crocbot skills list")}\` to see available skills.`,
      opts.json,
    );
  }
