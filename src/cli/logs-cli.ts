...
export function formatLogTimestamp(
  ...
  if (localTime) {
    ...
    return mode === "pretty" ? iso.slice(11, 19) : iso;
  }
  if (mode === "pretty") {
...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `crocbot logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("crocbot doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .option("--local-time, --localTime", "Show timestamps in local timezone instead of UTC", false)
    .addHelpText(
      "after",
      () =>
