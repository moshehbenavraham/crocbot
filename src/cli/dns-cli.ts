...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): crocbot.internal"));
      ...
      if (process.platform !== "linux") {
        throw new Error("dns setup is currently supported on Linux only");
      }
      ...
      const serverPath = path.join(confDir, "crocbot.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by crocbot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
