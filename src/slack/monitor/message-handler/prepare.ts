...
export async function prepareSlackMessage(params: {
  ...
  if (isDirectMessage) {
    ...
    if (ctx.dmPolicy !== "open") {
      ...
      if (!allowMatch.allowed) {
        // Read-only mode: pairing replies cannot be sent. Log and drop.
        if (ctx.dmPolicy === "pairing") {
          logVerbose(
            `slack read-only: drop dm from ${message.user} (pairing mode, no reply sent, ${allowMatchMeta})`,
          );
        } else {
          logVerbose(
  ...
  // Read-only mode: ack reactions are not added to Slack messages.
  const ackReactionPromise = null;
