...
export function listSignalAccountIds(cfg: crocbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: crocbotConfig): string {
...
function mergeSignalAccountConfig(cfg: crocbotConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: crocbotConfig): ResolvedSignalAccount[] {
