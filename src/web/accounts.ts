...
export function listWhatsAppAuthDirs(cfg: crocbotConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: crocbotConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: crocbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: crocbotConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: crocbotConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: crocbotConfig): ResolvedWhatsAppAccount[] {
