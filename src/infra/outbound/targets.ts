...
export function resolveOutboundTarget(params: {
  ...
  if (params.channel === INTERNAL_MESSAGE_CHANNEL) {
    return {
      ok: false,
      error: new Error(
        `Delivering to WebChat is not supported via \`${formatCliCommand("crocbot agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
      ),
    };
  }
...
export function resolveHeartbeatDeliveryTarget(params: {
  ...
  if (heartbeatAccountId && resolvedTarget.channel) {
    ...
    if (accountIds.length > 0) {
      const normalizedAccountIdValue = normalizeAccountId(heartbeatAccountId);
      const normalizedAccountIds = new Set(
        accountIds.map((accountId) => normalizeAccountId(accountId)),
      );
      if (!normalizedAccountIds.has(normalizedAccountIdValue)) {
        return {
          channel: "none",
          reason: "unknown-account",
          accountId: normalizedAccountIdValue,
          lastChannel: resolvedTarget.lastChannel,
          lastAccountId: resolvedTarget.lastAccountId,
        };
      }
      effectiveAccountId = normalizedAccountIdValue;
    }
  }
