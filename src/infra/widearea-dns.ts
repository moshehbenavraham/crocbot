...
export const WIDE_AREA_DISCOVERY_DOMAIN = "crocbot.internal.";
export const WIDE_AREA_ZONE_FILENAME = "crocbot.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*crocbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "crocbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "crocbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "crocbot-gw");
  ...
  records.push(`_crocbot-gw._tcp IN PTR ${instanceLabel}._crocbot-gw._tcp`);
  records.push(`${instanceLabel}._crocbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._crocbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; crocbot-content-hash: ${contentHash}\n${contentBody}`;
}
