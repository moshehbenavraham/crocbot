...
function candidateBinDirs(opts: EnsurecrocbotPathOpts): string[] {
  ...
  try {
    ...
    if (isExecutable(siblingcrocbot)) candidates.push(execDir);
  } catch {
    // ignore
  }
  ...
  if (isExecutable(path.join(localBinDir, "crocbot"))) candidates.push(localBinDir);
...
export function ensurecrocbotCliOnPath(opts: EnsurecrocbotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CROCBOT_PATH_BOOTSTRAPPED)) return;
  process.env.CROCBOT_PATH_BOOTSTRAPPED = "1";
