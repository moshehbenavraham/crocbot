...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    gitTotalSteps = channel === "dev" ? (needsCheckoutMain ? 9 : 8) : 7;
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "crocbot doctor",
        managerScriptArgs(manager, "crocbot", ["doctor", "--non-interactive"]),
        gitRoot,
        { CROCBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
