...
export class RequestBodyLimitError extends Error {
  ...
  constructor(init: { code: RequestBodyLimitErrorCode; message?: string }) {
...
function resolveValues(options: { maxBytes: number; timeoutMs?: number }): {
  maxBytes: number;
  timeoutMs: number;
} {
...
export async function readRequestBodyWithLimit(
  ...
  const { maxBytes, timeoutMs } = resolveValues(options);
...
export function installRequestBodyLimitGuard(
  ...
  const respond = (error: RequestBodyLimitError): void => {
  ...
  const trip = (error: RequestBodyLimitError): void => {
    if (tripped) {
      return;
    }
    tripped = true;
