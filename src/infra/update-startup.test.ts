...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-update-check-"));
    process.env.CROCBOT_STATE_DIR = tempDir;
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolvecrocbotPackageRoot } = await import("./crocbot-root.js");
    ...
    vi.mocked(resolvecrocbotPackageRoot).mockResolvedValue("/opt/crocbot");
