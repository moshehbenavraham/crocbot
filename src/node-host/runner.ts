...
function ensureNodePathEnv(): string {
  ensurecrocbotCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function runNodeHost(opts: NodeHostRunOptions): Promise<void> {
  ...
  const skillBins = new SkillBinsCache(async () => {
    const res = await client.request<{ bins?: unknown[] }>("skills.bins", {});
    const bins = Array.isArray(res?.bins) ? res.bins.map((bin: unknown) => String(bin)) : [];
    return bins;
  });
