...
2. **Appends to log file** - Writes a JSON line to `~/.crocbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.crocbot/logs/commands.log ~/.crocbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/crocbot`:
   ```
   /home/username/.crocbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
