...
This directory contains hooks that ship with Crocbot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
metadata:
  { "crocbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.crocbot/crocbot.json`:
...
2. Restart gateway: `pkill -9 -f 'crocbot.*gateway' && pnpm crocbot gateway`
3. Enable the hook: `crocbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.github.com/moshehbenavraham/crocbot/hooks
