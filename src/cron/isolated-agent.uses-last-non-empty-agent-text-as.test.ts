...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "crocbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".crocbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<crocbotConfig> = {},
): crocbotConfig {
  const base: crocbotConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".crocbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
