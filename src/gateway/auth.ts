...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.CROCBOT_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.CROCBOT_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or CROCBOT_GATEWAY_TOKEN)",
    );
  }
...
export async function authorizeGatewayConnect(params: {
  ...
  trustedProxies?: string[];
  tailscaleWhois?: TailscaleWhoisLookup;
  /** Optional rate limiter; when provided, failed attempts are tracked per IP. */
  rateLimiter?: AuthRateLimiter;
  /** Client IP for rate-limit tracking. Falls back to proxy-aware request IP. */
  clientIp?: string;
  /** Limiter scope; defaults to shared-secret scope. */
  ...
  if (auth.mode === "token") {
    ...
    if (!secretEqual(connectAuth.token, auth.token)) {
      limiter?.recordFailure(ip, rateLimitScope);
  ...
  if (auth.mode === "password") {
    ...
    if (!secretEqual(password, auth.password)) {
      limiter?.recordFailure(ip, rateLimitScope);
