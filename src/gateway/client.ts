...
export class GatewayClient {
  ...
  start() {
    ...
    if (url.startsWith("wss://") && this.opts.tlsFingerprint) {
      ...
      wsOptions.checkServerIdentity = ((_host: string, cert: CertMeta) => {
        ...
        return undefined;
        // oxlint-disable-next-line typescript/no-explicit-any -- ws ClientOptions.checkServerIdentity expects node:tls callback shape; our simplified CertMeta callback is compatible at runtime
      }) as any;
    }
  ...
  private sendConnect() {
    ...
    const storedToken = this.opts.deviceIdentity
      ...
      : null;
    // Prefer explicit token over stored device auth token
