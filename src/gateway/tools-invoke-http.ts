...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-crocbot-message-channel") ?? "");
  const accountId = getHeader(req, "x-crocbot-account-id")?.trim() || undefined;
  ...
  const allTools = createcrocbotTools({
