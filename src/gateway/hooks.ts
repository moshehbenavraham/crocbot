...
export function resolveHooksConfig(cfg: crocbotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: crocbotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-crocbot-token"] === "string" ? req.headers["x-crocbot-token"].trim() : "";
  if (headerToken) {
