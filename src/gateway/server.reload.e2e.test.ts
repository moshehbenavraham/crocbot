...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CROCBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.CROCBOT_SKIP_GMAIL_WATCHER;
    process.env.CROCBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CROCBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CROCBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CROCBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
