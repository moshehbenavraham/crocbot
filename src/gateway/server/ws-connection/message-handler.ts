...
function formatGatewayAuthFailureMessage(params: {
  ...
  const uiHint = "open a tokenized dashboard URL or provide the gateway auth token";
  ...
  const passwordHint = isCli
    ...
    : isControlUi || isWebchat
      ? "provide the gateway auth password"
    ...
    try {
      ...
      if (!client) {
        ...
        const helloOk = {
          ...
          server: {
            version: process.env.CROCBOT_VERSION ?? process.env.npm_package_version ?? "dev",
