...
describe("callGateway password resolution", () => {
  const originalEnvPassword = process.env.CROCBOT_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (originalEnvPassword == null) {
      ...
      process.env.CROCBOT_GATEWAY_PASSWORD = originalEnvPassword;
    }
  });
  ...
  it("prefers env password over local config password", async () => {
    process.env.CROCBOT_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
...
describe("callGateway token resolution", () => {
  const originalEnvToken = process.env.CROCBOT_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken == null) {
      ...
      process.env.CROCBOT_GATEWAY_TOKEN = originalEnvToken;
    }
  });

  it("uses remote token when remote mode uses url override", async () => {
    process.env.CROCBOT_GATEWAY_TOKEN = "env-token";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
