...
function collectBrowserControlFindings(cfg: crocbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("crocbot security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: crocbotConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: crocbotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && deep.gateway.ok === false) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("crocbot status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("crocbot security audit --deep")}".`,
    });
  }
