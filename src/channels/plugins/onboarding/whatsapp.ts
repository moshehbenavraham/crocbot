...
function setWhatsAppDmPolicy(cfg: crocbotConfig, dmPolicy: DmPolicy): crocbotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: crocbotConfig, allowFrom?: string[]): crocbotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: crocbotConfig, selfChatMode: boolean): crocbotConfig {
...
async function detectWhatsAppLinked(cfg: crocbotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so crocbot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for crocbot" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("crocbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
