...
function setSlackDmPolicy(cfg: crocbotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "crocbot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for crocbot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: crocbotConfig, allowFrom: string[]): crocbotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
