...
import { discovercrocbotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: crocbotPluginDefinition;
  register?: crocbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as crocbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as crocbotPluginDefinition;
...
export function loadcrocbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discovercrocbotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: crocbotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as crocbotPluginModule;
    } catch (err) {
