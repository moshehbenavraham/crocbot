...
async function ensurecrocbotExtensions(manifest: PackageManifest) {
  const extensions = manifest.crocbot?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing crocbot.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json crocbot.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensurecrocbotExtensions(manifest);
  } catch (err) {
  ...
  if (hasDeps) {
    ...
    const npmRes = await runCommandWithTimeout(["npm", "install", "--omit=dev", "--silent", "--no-audit", "--no-fund", "--ignore-scripts"], {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-npm-pack-"));
