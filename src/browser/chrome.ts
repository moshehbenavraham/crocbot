...
import { DEFAULT_CROC_BROWSER_COLOR, DEFAULT_CROC_BROWSER_PROFILE_NAME } from "./constants.js";
...
export async function launchCrocChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CROC_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateCrocProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🐊 croc browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`croc browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`croc browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🐊 croc browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
