...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".croc-profile-decorated");
}
 ...
 */
export function decorateCrocProfile(userDataDir: string, opts?: { name?: string; color?: string }) {
  const desiredName = opts?.name ?? DEFAULT_CROC_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CROC_BROWSER_COLOR).toUpperCase();
