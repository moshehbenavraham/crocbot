...
const DEFAULT_PENDING_MAX_OUTPUT = clampNumber(
  readEnvInt("CROCBOT_BASH_PENDING_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (host === "node") {
        ...
        if (hostAsk === "on-miss" && hostSecurity === "allowlist" && analysisOk) {
          try {
            const approvalsSnapshot = await callGatewayTool<{ file?: unknown } | null>(
        ...
        if (requiresAsk) {
          ...
          void (async () => {
            ...
            try {
              const decisionResult = await callGatewayTool<{ decision?: string } | null>(
