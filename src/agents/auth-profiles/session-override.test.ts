...
describe("resolveSessionAuthProfileOverride", () => {
  it("keeps user override when provider alias differs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-auth-"));
    ...
    try {
      ...
      if (prevStateDir === undefined) delete process.env.CROCBOT_STATE_DIR;
      else process.env.CROCBOT_STATE_DIR = prevStateDir;
