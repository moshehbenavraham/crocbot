...
function resolveExecConfig(cfg: crocbotConfig | undefined) {
...
export function createcrocbotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createcrocbotReadTool(freshReadTool)];
    }
