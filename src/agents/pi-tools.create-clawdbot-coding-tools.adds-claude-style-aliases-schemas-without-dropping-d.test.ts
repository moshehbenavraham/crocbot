...
const defaultTools = createcrocbotCodingTools();
...
describe("createcrocbotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createcrocbotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from crocbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "crocbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "crocbot-workspace"),
    ...
    const tools = createcrocbotCodingTools({ sandbox });
