...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-embedded-agent-"));
...
const ensureModels = (cfg: crocbotConfig) => ensurecrocbotModelsJson(cfg, agentDir) as unknown;
