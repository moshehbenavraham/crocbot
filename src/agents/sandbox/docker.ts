...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "crocbot.sandbox=1");
  args.push("--label", `crocbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `crocbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `crocbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "crocbot.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) {
    return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`crocbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`crocbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("crocbot sandbox recreate --all");
}
