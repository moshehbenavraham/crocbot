...
function sanitizeHistoryMessage(message: unknown): { message: unknown; truncated: boolean } {
  ...
  if (typeof entry.content === "string") {
    ...
    const updated = entry.content.map((block: unknown) => sanitizeHistoryContentBlock(block));
    entry.content = updated.map((item: { block: unknown }) => item.block);
    truncated ||= updated.some((item: { truncated: boolean }) => item.truncated);
  }
  if (typeof entry.text === "string") {
    const res = truncateHistoryText(entry.text);
    entry.text = res.text;
...
async function isSpawnedSessionAllowed(params: {
  ...
  try {
    const list = await callGateway<{ sessions?: Array<Record<string, unknown>> }>({
    ...
    return sessions.some(
      (entry: Record<string, unknown>) => entry?.key === params.targetSessionKey,
    );
  } catch {
    return false;
  }
