...
async function buildReminderContextLines(params: {
  ...
  try {
    const res = await callGatewayTool<{ messages?: unknown[] }>(
      "chat.history",
      params.gatewayOpts,
      {
    ...
    const parsed = messages
      .map((msg: unknown) => extractMessageText(msg as ChatMessage))
      .filter((msg: unknown): msg is { role: string; text: string } => Boolean(msg));
