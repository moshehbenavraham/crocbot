...
/**
 * Cleanup suspended crocbot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: crocbotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crocbot-cli-images-"));
  const paths: string[] = [];
